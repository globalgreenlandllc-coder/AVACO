...
export const gateway = {
  ...
  createAnalysis: (input: { audioUrl: string; userId: string }) =>
    call<{ id: string; status: string }>("/api/v1/analyses", {
      method: "POST",
      body: JSON.stringify({ audio_url: input.audioUrl, type: "both", mode: "async", external_user_id: input.userId, consent: true }),
    }),
