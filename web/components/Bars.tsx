          ...
          <>
            <span className="flex items-baseline justify-between gap-3">
              <span className="flex items-baseline gap-2 font-medium">
                {row.name}
                {expandable && <span className={`no-print text-xs text-muted transition-transform ${isOpen ? "rotate-90" : ""}`} aria-hidden>›</span>}
              </span>
            ...
            {row.text && !isOpen && <span className={`mt-2 block text-sm leading-relaxed text-ink-2 ${row.zone === "background" || !expandable ? "" : "print:hidden"}`}>{row.text}</span>}
          </>
        );
