/** Blob upload token for the partner. Their private link token is the credential. */
...
export async function POST(req: Request, ctx: { params: Promise<{ token: string }> }) {
  try {
    if (!(await matchByToken((await ctx.params).token))) return json({ error: "not_found", message: "Not found" }, 404);
